...
#script taken from https://github.com/chrishah/genomisc/blob/master/popogeno/vcf_2_div.py
...
vcf = vcf.Reader(filename=args.VCF)
...
#        print "currently %s elements in whitelist" %len(whitelist)
        ...
        if str(record.ID) not in whitelist:
...
#            continue
            ...
            for key in list(temp_pop.keys()):
                popcounts[key].append(temp_pop[key])
        if args.verbose:
            print("%s\t%s" %(str(record.CHROM)+"\t"+str(record.POS)+"\t"+str(record.ID),verbose_out))

if not args.whitelist:
    if (args.random_n == 0) or (len(popcounts[list(popcounts.keys())[0]]) <= args.random_n):
        rand = list(range(len(popcounts[list(popcounts.keys())[0]])))
#        outfile = "full_"+str(len(rand))+".bayenv"
        ...
        rand = random.sample(list(range(len(popcounts[list(popcounts.keys())[0]]))),args.random_n)
#        outfile = "random_"+str(len(rand))+".bayenv.in"
    ...
    print("\n%i (%.2f %%) of the loci specified in the whitelist were found" %(len(popcounts[list(popcounts.keys())[0]]), float(len(popcounts[list(popcounts.keys())[0]]))/whitelist_count*100))
    if len(popcounts[list(popcounts.keys())[0]]) != whitelist_count:
